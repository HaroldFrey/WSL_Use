...
D:\FPGA_Self_Stduy\WSL_Use\    ← Windows 端：存放文档（本项目实际路径）
    ...
    ├── 06-iverilog与GTKWave安装.md ← HDL 仿真器 + 波形查看器
    └── ...
...
`/mnt` 是 Linux 里的"**挂载点**"目录（mount 的缩写）——把别的设备"接"进来看。WSL 会自动把 Windows 的每个盘挂到 `/mnt` 下：

```
Linux 视角                对应 Windows 真实位置
────────────────────────────────────────────────
/mnt/c/...        →      C:\...   （你的 C 盘）
/mnt/d/...        →      D:\...   （你的 D 盘）
~/  (/home/harold) →     在 D:\WSL\Ubuntu\ext4.vhdx 里面（虚拟磁盘内）
```

- **`ls /mnt/c/`** = 在 Linux 里浏览 C 盘内容
- **`ls ~/`** = 在 Linux 自己的"系统盘"里（虚拟磁盘，物理在 D 盘）
- 想从 Windows 拷文件进 WSL，走 `/mnt/` 路径；想放 WSL 内部，走 `~`

> 💡 一句话记忆：**"装到 D 盘" = Ubuntu 系统盘在 D 盘的 vhdx 里；"能看到 C 盘" = WSL 可以访问（挂载）Windows 的所有盘**——访问功能与安装在哪儿无关。
